...
- **Platform detection** (`index.php:16-45`): auto-detects the serverless platform at runtime via environment variables (`SCF`, `FC`, `FG`, `CFC`, `Heroku`, `Vercel`, `Replit`, `Normal`).
- **Platform adapters** in `platform/`: each handles request parsing, config I/O, and response formatting for that specific hosting environment. `Normal.php` handles traditional PHP hosts/VPS.
...
- The `$EnvConfigs` bitmask in `common.php:9-72` encodes per-key metadata: common vs inner (per-disk), shown vs hidden, base64-encoded, switch vs string.
...
- `main($path)` in `common.php:138` is the core router/dispatcher.
- Paths are normalized with `path_format()` to forward slashes, no double slashes.
- Admin auth uses a cookie + localStorage dual mechanism (`adminpass2cookie` / `adminpass2storage` / `compareadminmd5`) with a CSRF token (`_admin` form field).
...
- The update mechanism in `platform/Normal.php:OnekeyUpate()` downloads a zip/tarball from GitHub/Gitee, extracts it, copies `.data/config.php` to the new code, then replaces all files.
